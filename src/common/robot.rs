...
impl LocalRobot {
        ...
        if let Some(components) = cfg.components.as_ref() {
            ...
            let b = match r {
                Some(r) => {
                    ...
                    let ctor = registry.get_board_constructor(r.get_model())?;
    ...
    fn insert_resources(
        ...
        mut registry: ComponentRegistry<'_, '_, '_>,
    ) -> anyhow::Result<()> {
        ...
        while !components.is_empty() && num_iterations < max_iterations {
            ...
            if !inserted_resources.contains(&resource_key) {
                match registry.get_dependency_function(c_type_static, &model) {
    ...
    fn insert_resource(
        ...
        cfg: ConfigType,
        deps: Vec<Dependency>,
        registry: &mut ComponentRegistry<'_, '_, '_>,
    ) -> anyhow::Result<()> {
