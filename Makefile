...
build-qemu:
	cd examples && cargo build  --example esp32_static  --features qemu --target=xtensa-esp32-espidf -Zbuild-std=std,panic_abort && cargo espflash save-image --features qemu --merge --chip esp32 target/xtensa-esp32-espidf/debug/debug.bin -T esp32/partitions.csv -s 4M  --example esp32_static --target=xtensa-esp32-espidf -Zbuild-std=std,panic_abort
